...
def formatquery():
    query = subprocess.getstatusoutput(
        'zenity --entry --title="Query Box" --text="Enter your query" --entry-text "Query"')[1].split("\n")[1]
...
def geturl():
    Utubeurl = subprocess.getstatusoutput(
        'zenity --entry --title="URL Box" --text="Enter URL" --entry-text "Youtube Url"')[1].split("\n")[1]
    return(Utubeurl)


def getPlaylistdownload():
    download_type = subprocess.getstatusoutput(
        'zenity --title="vidoe download type" --list  --column "download type" "Download all" "choose some" "settings"')[1].split("\n")[1]
...
if __name__ == '__main__':
    ...
    download_type = subprocess.getstatusoutput(
        'zenity --title="vidoe quality" --list  --column "download type" "stand alone link" "playlist" "from url"')[1].split("\n")
    ...
    if download_type == 'stand alone link':
        ...
        videos_choice = subprocess.getstatusoutput(
            'zenity --title="""video""" --list --radiolist --column """ """ --column """video list""" FALSE """'+videos_standalone+'"""')[1].split("\n")[1]
        downloads = c.selectLink(videos_choice)
        quality = list(downloads.keys())
        quality = "' FALSE '".join(quality)
        quality_choice = subprocess.getstatusoutput(
            "zenity --title='video quality' --list --radiolist --column '' --column 'video quality' FALSE 'default' FALSE '"+quality+"'")[1].split("\n")[1]
        ...
        playlist_choice = subprocess.getstatusoutput(
            'zenity --title="""video""" --list --radiolist --column """ """ --column """Playlist list""" FALSE """'+videos_playlist+'"""')[1].split("\n")[1]
        ...
        if '&list' in htmlurl:
            ...
            while download_type == 'settings':
                quality_setting = subprocess.getstatusoutput(
                    'zenity --title="video download type" --list  --column "quality settings" "default" "manual"')[1].split("\n")[1]
