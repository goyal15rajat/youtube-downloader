...
class crawlUrl(object):
    ...
    def requestUrl(self, htmlUrl):
        req = urllib.request.Request(
            htmlUrl, headers={'User-Agent': 'Mozilla/5.0'})
        return(req)
    ...
    def crawlQualitylink(self, quality_choice):
        ...
        save_video = "http://www.save-video.com/" + \
            self.downloads[quality_choice]+"#"
        url = save_video
    ...
    def getPlaylist_videos(self, playlist_choice):
        ...
        divs = soup.find('div', attrs={
                         'class': 'playlist-videos-container yt-scrollbar-dark yt-scrollbar'})
...
# for downloading playlist from url
    ...
    def downloadallPlaylist(self, quality):
        if quality != 'default':
            ...
            for keys in self.title_list:
                self.findQuality(self.title_list[keys])
    ...
    def selectPlaylist_videos(self, quality):
        ...
        playlist_choice = subprocess.getstatusoutput(
            'zenity --title="""video""" --list --checklist --column """ """ --column """Playlist list""" FALSE """'+videos_playlist+'"""')[1].split("\n")[1].split("|")
        print(playlist_choice)
        if quality != 'default':
            for title in playlist_choice:
                self.findQuality(self.title_list[title])
                self.crawlQualitylink(quality)
        else:
            for title in playlist_choice:
                self.findQuality(self.title_list[title])
    ...
    def downloadLink(self, req):
        ...
        if temp1 == None:
            urllib.request.urlretrieve(temp2, download_name, self.reporthook)
        else:
            r = requests.get(temp1)
            urllib.request.urlretrieve(
                r.history[0].url, download_name, self.reporthook)
